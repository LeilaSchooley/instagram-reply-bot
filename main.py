...
def generate_response(messages, last_message):
    """
    Generate a response using the last 100 messages as context and the most recent message for the reply.

    Parameters:
    - messages: List of the last 100 messages (conversation history).
    - last_message: The latest message that needs to be replied to.
    ...
    # Add the last message (the one to reply to)
...
def send_message(page):
    ...
    reply = generate_response(messages, last_message_text)
